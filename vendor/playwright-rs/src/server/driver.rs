...
/// 1. PLAYWRIGHT_DRIVER_PATH environment variable (explicit user/application override)
/// 2. PLAYWRIGHT_NODE_EXE and PLAYWRIGHT_CLI_JS environment variables (explicit override)
/// 3. Bundled driver downloaded by build.rs (matches official bindings)
/// 4. User cache populated by `playwright-rs install` (stable across cargo install)
...
pub fn get_driver_executable() -> Result<(PathBuf, PathBuf)> {
    // Explicit runtime configuration must win. Distributable applications use
    // this to select the driver shipped beside their executable even when an
    // unrelated, stale driver exists in the per-user cache.
...
mod tests {
    ...
    static DRIVER_ENV_LOCK: Mutex<()> = Mutex::new(());
    ...
    #[test]
    fn explicit_driver_path_has_highest_priority() {
        let _lock = DRIVER_ENV_LOCK.lock().unwrap();
        let temp = tempfile::tempdir().unwrap();
        let node_name = if cfg!(windows) { "node.exe" } else { "node" };
        std::fs::create_dir_all(temp.path().join("package")).unwrap();
        std::fs::write(temp.path().join(node_name), b"").unwrap();
        std::fs::write(temp.path().join("package").join("cli.js"), b"").unwrap();

        let previous = std::env::var_os("PLAYWRIGHT_DRIVER_PATH");
        // SAFETY: environment mutation is serialized by DRIVER_ENV_LOCK and
        // restored before this test releases the lock.
        unsafe { std::env::set_var("PLAYWRIGHT_DRIVER_PATH", temp.path()) };
        let result = get_driver_executable();
        // SAFETY: same serialized scope as the set above.
        unsafe {
            match previous {
                Some(value) => std::env::set_var("PLAYWRIGHT_DRIVER_PATH", value),
                None => std::env::remove_var("PLAYWRIGHT_DRIVER_PATH"),
            }
        }

        let (node, cli) = result.unwrap();
        assert_eq!(node, temp.path().join(node_name));
        assert_eq!(cli, temp.path().join("package").join("cli.js"));
    }
