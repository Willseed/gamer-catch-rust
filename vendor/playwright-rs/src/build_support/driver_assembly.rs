...
fn assemble_driver(
    ...
    let result = (|| {
        let npm_url = npm_core_url(driver_version);
        let npm_bytes = http_get(&npm_url)?;
        let npm_checksum = npm_core_sha512(driver_version).ok_or_else(|| {
            io::Error::other(format!(
                "no pinned playwright-core checksum for version {driver_version}"
            ))
        })?;
        verify_sha512(&npm_bytes, npm_checksum, &npm_url)?;
        extract_npm_package(&npm_bytes, &tmp)?;

        let node_url = node_archive_url(NODE_VERSION, triple);
        let node_bytes = http_get(&node_url)?;
        let node_checksum = node_archive_sha256(NODE_VERSION, triple).ok_or_else(|| {
            io::Error::other(format!(
                "no pinned Node checksum for version {NODE_VERSION} and {triple}"
            ))
        })?;
        verify_sha256(&node_bytes, node_checksum, &node_url)?;
        extract_node_binary(
            &node_bytes,
            NODE_VERSION,
            triple,
...
#[allow(dead_code)]
fn verify_sha256(bytes: &[u8], expected: &str, source: &str) -> std::io::Result<()> {
    use sha2::{Digest, Sha256};
    verify_digest(Sha256::digest(bytes).as_slice(), expected, source)
}

#[allow(dead_code)]
fn verify_sha512(bytes: &[u8], expected: &str, source: &str) -> std::io::Result<()> {
    use sha2::{Digest, Sha512};
    verify_digest(Sha512::digest(bytes).as_slice(), expected, source)
}

#[allow(dead_code)]
fn verify_digest(actual: &[u8], expected: &str, source: &str) -> std::io::Result<()> {
    use std::fmt::Write;
    ...
    if actual_hex != expected {
        return Err(std::io::Error::other(format!(
            "checksum mismatch for {source}: expected {expected}, got {actual_hex}"
        )));
    }
    Ok(())
}
